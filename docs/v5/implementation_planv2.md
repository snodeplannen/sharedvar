...
Alle open vragen zijn beantwoord. Het definitieve ontwerp is vastgelegd in `SharedValueV5/ARCHITECTURE_NL.md`.

| Beslissing | Antwoord |
|---|---|
| FlatBuffers trade-off | ✅ Acceptabel — V4 blijft beschikbaar voor ultra-HFT |
| COM API stijl | ✅ **Beide** — Cursor-model + Parameter-Array model |
| Bidirectioneel | ✅ Ja, volledig bidirectioneel (P2C + C2P) |
| Multi-table | ✅ Ja, DataSet model met meerdere tabellen |
| Schema lock-out | ✅ Ja, `LockSchema()` bevriest permanent |
| Python wrapper | ✅ C-Extension DLL (`.pyd`) |
